...
const LineChart = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            console.log(`Requesting data from: http://localhost:5000/api/events/${selectedDeviceId}`);
            const response = await axios.get(`http://localhost:5000/api/events/${selectedDeviceId}`, { signal });
            ...
            if (Array.isArray(response.data)) {
                ...
                if (extractedData.length > 0) {
                    ...
                    console.warn("No valid timestamp-value pairs found in response:", response.data);
                    setEventData([]);
                }
